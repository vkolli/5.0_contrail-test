...
class PerfBase(test_v1.BaseTestCase_v1,VerifySvcMirror):
   ...
   #     action_list = self.chain_si(self.vm_num,self.svc_int_prefix,self.inputs.project_name)
    ...
    def update_encap_mode(self,host,mode):
        ...
        host = self.inputs.bgp_control_ips[0]
 
        out = self.inputs.run_cmd_on_server(host, cmd , container='control')

        if not re.findall("Updated",out):
            self.logger.error("Encapsulation not updated :%s "%out)
            return False
        res = ('Encapsulation Type :%s\n'%mode)
        self.logger.info("Encapsulation Type :%s "%res)
        return True
